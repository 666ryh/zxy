...
export function createAuth({send,clock=Date.now,accounts=createAccounts()}){
 ...
 async verify(email,code){email=normalize(email);const record=challenges.get(email);if(!record||record.pending||record.verifying||record.expires<=clock())throw Error('验证码无效或已过期');if(record.attempts>=5)throw Error('验证码尝试过多，请重新获取');record.attempts++;if(typeof code!=='string'||!timingSafeEqual(hash(code),record.digest))throw Error('验证码不正确');record.verifying=true;try{cleanup();const token=randomBytes(32).toString('hex');await accounts.login(email,token,clock());challenges.delete(email);return token;}catch(e){record.verifying=false;throw e;}},
 session(token){return accounts.session(token,clock());},
 logout(token){return accounts.logout(token);}
 };
}
