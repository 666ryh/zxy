...
const services=[['salary','payroll','薪资设置','底薪、晚辅与招生提成'],['attendance','attendance','我的考勤','记录每一次认真'],['backup','download','数据与备份','给教学手账留一份副本'],['support','chat','联系辉辉','由辉辉本人微信回复'],['about','star','关于课笺','我的紫色教学手账']];
function open(action){if(action==='attendance')navigate('attendance');else{if(action==='salary')state.month=dateKey().slice(0,7);state.modal={type:action};}}
</script>
<template>
 <view class="profile-page">
  ...
  <view class="profile-body">
   ...
   <button role="button" tabindex="0" class="huihui-card" @click="state.modal={type:'support'}"><view class="huihui-avatar">辉</view><view><text class="huihui-title">有问题，就来找辉辉</text><text class="huihui-sub">你的专属人工客服 · 微信联系</text></view><AppIcon name="arrow" :size="17"/></button>
