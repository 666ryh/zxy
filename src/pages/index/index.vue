...
onBackPress(()=>{if(appearance.open){appearance.open=false;return true;}if(state.supportOpen){state.supportOpen=false;return true;}if(state.modal){state.modal=null;return true;}if(state.profileEditing){state.profileEditing=false;return true;}return false;});
function escape(e){if(e.key==='Escape'){if(appearance.open){appearance.open=false;return;}if(state.supportOpen){state.supportOpen=false;return;}if(state.modal)state.modal=null;else state.profileEditing=false;}}
